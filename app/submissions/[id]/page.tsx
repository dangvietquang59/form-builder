    ...
    <div className="container mx-auto py-6">
      ...
      {submissions.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-6">Share the form link to start collecting responses</p>
              ...
              <CardContent>
                <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(submission.data).map(([key, value]: [string, any]) => {
                    const component = form.components.find((c: any) => c.key === key)
                    if (!component) return null

                    return (
                      <div key={key} className="space-y-1">
                        <dt className="text-sm font-medium">{component.label.en}</dt>
                        <dd className="text-sm">
                          {Array.isArray(value)
                            ? value
                                .map((v: string) => {
                                  const option = component.options?.find((o: any) => o.value === v)
                                  return option ? option.en : v
                                })
                                .join(", ")
                            : (() => {
                                if (component.type === "select" || component.type === "radio") {
                                  const option = component.options?.find((o: any) => o.value === value)
